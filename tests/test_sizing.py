...
def test_pace_factor_behind_near_end_of_week_scales_up():
    # Nothing won, one day left, goal=100 → very behind pace.
...
def test_pace_factor_on_pace_is_near_one():
    # Halfway through the week with half the goal in the bag.
...
def test_size_bet_respects_max_cap():
    # Big Kelly, small bankroll → should clamp to MAX_STAKE_PCT_OF_BANKROLL.
    stake = size_bet(
        kelly_full=1.0,
        bankroll=1000,
        risk_tolerance="high",
...
def test_size_bet_medium_on_pace_matches_base_formula():
    ...
    # Allow small pace-factor wiggle because "on pace" is approximate.
